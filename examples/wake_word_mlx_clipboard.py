...
def selective_print(*args, **kwargs):
    ...
    allowed_prefixes = [
        ...
        "Speech detected", "🎤 Final", "⏱️", "📋 Copied"
    ]
...
"""
Wake word detection with MLX Whisper transcription and clipboard copying example.

This script demonstrates how to use the wake word detection functionality
to trigger MLX Whisper transcription, automatically copying the results
to the clipboard (macOS). It implements a two-stage activation approach:

1. First stage: Only wake word detection runs continuously
2. Second stage: VAD and MLX Whisper transcription activated only after wake word detection
3. Final stage: Transcription results are automatically copied to clipboard

Running modes:
* Default (quiet): Shows only user output with emojis, hides log messages
  $ python -m examples.wake_word_mlx_clipboard
  
* Verbose mode: Shows detailed INFO-level logs and user output
  $ python -m examples.wake_word_mlx_clipboard --verbose
  
* Debug mode: Shows all DEBUG-level logs for detailed troubleshooting
  $ python -m examples.wake_word_mlx_clipboard --debug
"""
...
def main():
    """Run the wake word detection with MLX Whisper transcription and clipboard copying example."""
    # We already set up quiet mode and print redirection at the top of the file
    global is_quiet_mode, seen_transcriptions
    # Parse command line arguments
    parser = argparse.ArgumentParser(description="Wake word detection with MLX Whisper transcription and clipboard copying example")
    ...
    parser.add_argument("--model", type=str, default="whisper-large-v3-turbo",
                        help="MLX Whisper model to use (default: whisper-large-v3-turbo)")
    ...
    parser.add_argument("--no-quick-mode", action="store_true",
                        help="Disable quick/parallel mode for transcription (more accurate but slower)")
    ...
    LoggingModule.initialize(
        ...
        file_path="logs/wake_word_mlx_clipboard.log",
    ...
    # Register transcription module with MLX Whisper engine
    transcription_module = TranscriptionModule.register(
        command_dispatcher, 
        event_bus,
        default_engine="mlx_whisper",  # Use MLX Whisper instead of OpenAI
    ...
    # Configure MLX-specific transcription
    TranscriptionModule.configure(
        ...
        streaming=not args.no_quick_mode,
        options={
    ...
    logger.info(f"Using MLX Whisper '{args.model}' for transcription")
    ...
    print(f"🧠 Using MLX Whisper '{args.model}' for transcription")
