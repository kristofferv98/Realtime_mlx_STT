...
"""
Wake word detection with OpenAI transcription and clipboard copying example.

This script demonstrates how to use the wake word detection functionality
to trigger OpenAI-based transcription, automatically copying the results
...
* Default (quiet): Shows only user output with emojis, hides log messages
  $ python -m examples.wake_word_openai_clipboard
  
* Verbose mode: Shows detailed INFO-level logs and user output
  $ python -m examples.wake_word_openai_clipboard --verbose
  
* Debug mode: Shows all DEBUG-level logs for detailed troubleshooting
  $ python -m examples.wake_word_openai_clipboard --debug
...
def main():
    """Run the wake word detection with OpenAI transcription and clipboard copying example."""
    # We already set up quiet mode and print redirection at the top of the file
    global is_quiet_mode, seen_transcriptions
    # Parse command line arguments
    parser = argparse.ArgumentParser(description="Wake word detection with OpenAI transcription and clipboard copying example")
    ...
    LoggingModule.initialize(
        ...
        file_path="logs/wake_word_openai_clipboard.log",
