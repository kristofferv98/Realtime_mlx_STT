...
class AudioToTextRecorderClient:
    ...
    voice activity, and then transcribing the captured audio using a remote
    speech-to-text server with MLX backend.
    
    This client is designed to work with a server that uses the MLX-optimized
    Whisper large-v3-turbo model running on Apple Silicon. It provides a simple
    way to connect to and interact with a remote transcription service without
    needing to run the model locally.
    
    Note: This client requires a compatible server that supports the MLX backend.
    """
    ...
    def start_server(self):
        ...
        if self.mlx_model_path:
            args += ['--mlx_model_path', self.mlx_model_path]
